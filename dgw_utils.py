...
def build_course_list(ctx, institution, requirement_id) -> dict:
  ...
  #   xxx_head productions, where the label is attached to the production, not the list. Leaving
  #   this note here in the hope of finding it the next time the question of whether to look for a
  #   label here comes up.
  ...
  scribed_courses += get_scribed_courses(first_course, other_courses)
  ...
  if (len(scribed_courses) == 1) and \
     (scribed_courses[0][0] == '@') and \
     (scribed_courses[0][1] == '@'):
     # Construct an active_courses list that captures the essence of @ @
     scribed_course = scribed_courses[0]
     active_courses = [000000, 0, 'ANY', 'COURSE', scribed_course[2]]
  else:
